...
export default function About() {
  ...
  // Header entries (with header_image)
    ...
    <>
      ...
      {headerEntries.length > 0 && (
        <div style={{
          ...
          <div style={{ position: 'absolute', inset: 0 }}>
            {headerEntries.map((entry, i) => (
              <div key={i} style={{
                ...
                {entry.header_image && (
                  <img src={entry.header_image} alt={entry.header_title || ''} style={{
                    position: 'absolute', inset: 0, width: '100%', height: '100%',
          ...
          {/* Content */}
          <div style={{
            position: 'absolute', bottom: 'clamp(40px, 10vh, 80px)', left: '50%',
            ...
            <h1 style={{
              ...
              fontWeight: 900, lineHeight: 1.1, color: '#fff', margin: 0,
            ...
            {headerEntries[currentSlide]?.header_subtitle && (
              <p style={{
                ...
                fontWeight: 500, color: 'rgba(255,255,255,0.9)', marginTop: 8,
          ...
          {headerEntries.length > 1 && (
            <div style={{
              ...
              transform: 'translateX(-50%)', zIndex: 4, display: 'flex', gap: 8
            }}>
          ...
          {headerEntries.length > 1 && (
            <>
              <button onClick={() => goToSlide((currentSlide - 1 + headerEntries.length) % headerEntries.length)}
                style={{
                  position: 'absolute', top: '50%', left: 'clamp(12px, 2vw, 24px)',
                  transform: 'translateY(-50%)', zIndex: 4, width: 44, height: 44,
              ...
              <button onClick={() => goToSlide((currentSlide + 1) % headerEntries.length)}
                style={{
                  position: 'absolute', top: '50%', right: 'clamp(12px, 2vw, 24px)',
                  transform: 'translateY(-50%)', zIndex: 4, width: 44, height: 44,
                  borderRadius: '50%', background: 'rgba(255,255,255,0.1)',
                  backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.2)',
                  color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-4">
        {entries.map((entry, index) => (
          <div key={entry.id || index}>
            <div className="about-card" data-animate="true" style={{
              opacity: 0, transform: 'translateY(40px)', marginBottom: 80,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {entry.section_title && (
                    <h2 style={{
                      ...
                      fontWeight: 800, lineHeight: 1.15, color: '#1d1d1f',
                      margin: '0 0 8px 0', letterSpacing: '-0.02em'
                    }}>{entry.section_title}</h2>
                  )}

                  {entry.section_subtitle && (
                    <p style={{
                      fontFamily: "'Inter', sans-serif", fontSize: 18, fontWeight: 500,
                      color: '#666', margin: '0 0 24px 0', lineHeight: 1.5
                    }}>{entry.section_subtitle}</p>
                  )}
                  ...
                  {entry.cta_text && entry.cta_link && (
                    <Link href={entry.cta_link} style={{
            ...
            {index < entries.length - 1 && (
              <div style={{ width: '100%', height: 1, background: '#e5e5ea', marginBottom: 64 }} />
            )}
          </div>
        ))}
      </div>
      ...
      <style jsx>{`
        ...
        @media (max-width: 768px) {
          .header-arrow-hover { opacity: 1 !important; pointer-events: auto !important; width: 36px !important; height: 36px !important; font-size: 12px !important; }
          .about-card { margin-bottom: 48px !important; }
        }
        @media (max-width: 480px) {
          .about-card { margin-bottom: 36px !important; }
        }
      `}</style>
    </>
  );
