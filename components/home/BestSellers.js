...
export default function BestSellers({ products = [] }) {
  ...
  useEffect(() => {
    async function fetchItems() {
      try {
        ...
        const filtered = all.filter(p => p.is_best === true || p.is_best === 1 || p.is_best === 'true');
        filtered.sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));
        setItems(filtered.slice(0, maxProducts));
      } catch (e) {
        const filtered = products.filter(p => p.is_best === true || p.is_best === 1);
        setItems(filtered.slice(0, maxProducts));
      }
      setLoading(false);
    }
  ...
  const getImageUrl = (product) => {
    if (product.img?.trim()) return product.img;
    ...
    return (
      <section style={{ padding: '32px 0', background: '#fff' }}>
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          <h2 style={{ fontFamily: "'Manrope', sans-serif", fontWeight: 700, fontSize: 22, color: '#1d1d1f' }}>Best Sellers</h2>
        </div>
    ...
    return (
      <section style={{ padding: '32px 0', background: '#fff' }}>
        ...
        <div style={{ textAlign: 'center', padding: '60px 20px', maxWidth: 400, margin: '0 auto' }}>
          ...
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 15, color: '#86868b' }}>No best sellers right now</p>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, color: '#b0b0b5', marginTop: 6 }}>Top picks coming soon — stay tuned</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section style={{ padding: '32px 0', maxWidth: 1400, margin: '0 auto', background: '#fff' }}>
      <div style={{ textAlign: 'center', marginBottom: 20, padding: '0 8px' }}>
        <h2 style={{ fontFamily: "'Manrope', sans-serif", fontWeight: 700, fontSize: 22, color: '#1d1d1f', letterSpacing: '-0.3px' }}>Best Sellers</h2>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1 }} className="bs-grid">
        {items.map(product => (
          <ProductCard key={product.id} product={product} getImageUrl={getImageUrl} getSlug={getSlug} fetchColors={fetchColors} colorCache={colorCache} />
        ))}
      </div>
      <style jsx>{`
        @media (min-width: 768px) { .bs-grid { grid-template-columns: repeat(3, 1fr) !important; } }
        @media (min-width: 1024px) { .bs-grid { grid-template-columns: repeat(4, 1fr) !important; } }
      `}</style>
    </section>
  );
}
...
function ProductCard({ product, getImageUrl, getSlug, fetchColors, colorCache }) {
  ...
  const onTouchMove = (e) => { if (!isDragging) return; setDragOffset(e.touches[0].clientX - startX.current); };
  const onTouchEnd = () => {
    setIsDragging(false);
    if (Math.abs(dragOffset) > 60) {
      dragOffset < 0 && currentImage < totalImages - 1 ? navigate(currentImage + 1) : dragOffset > 0 && currentImage > 0 ? navigate(currentImage - 1) : null;
    }
    setDragOffset(0);
  };

  return (
    <div ref={cardRef} style={{ position: 'relative', background: '#fff', cursor: 'pointer', overflow: 'hidden', transition: 'transform 0.3s' }}
      className="bs-card"
    ...
    >
      <Link href={`/product/${encodeURIComponent(slug)}`} style={{ textDecoration: 'none', color: 'inherit', display: 'flex', flexDirection: 'column', height: '100%' }}>
        <div style={{ position: 'relative', aspectRatio: '4/5', background: '#f5f5f7', overflow: 'hidden', marginBottom: 6 }}>
          ...
          {!isOut && (product.is_best === true || product.is_best === 1) && (
            <span style={{ position: 'absolute', top: 4, left: 4, zIndex: 5, padding: '2px 7px', fontFamily: "'Sora', sans-serif", fontWeight: 600, fontSize: 8, textTransform: 'uppercase', background: '#fff', color: '#1d1d1f', letterSpacing: '0.5px', borderRadius: 1 }}>Best Seller</span>
          )}
          {!isOut && (product.is_on_sale === true || product.is_on_sale === 1) && (
            <span style={{ position: 'absolute', top: 4, left: 4, zIndex: 5, padding: '2px 7px', fontFamily: "'Sora', sans-serif", fontWeight: 600, fontSize: 8, textTransform: 'uppercase', background: '#fff', color: '#d70015', letterSpacing: '0.5px', borderRadius: 1 }}>Sale</span>
          )}
          {isOut && (
        ...
        <div style={{ padding: '4px 6px 6px' }}>
          ...
          {colors.length > 0 && (
            <div style={{ display: 'flex', gap: 5, justifyContent: 'center', marginTop: 6, flexWrap: 'wrap' }}>
              {colors.map((c, i) => (
                <span key={i} title={c.color_name} style={{ width: 11, height: 11, borderRadius: '50%', backgroundColor: c.color_code || '#ccc', border: '1px solid rgba(0,0,0,0.08)', cursor: 'pointer' }} />
              ))}
            </div>
          )}
        </div>
      ...
      <style jsx>{`
        ...
        @media (hover: hover) { .bs-card:hover { transform: translateY(-2px); z-index: 2; box-shadow: 0 8px 25px rgba(0,0,0,0.12); } }
      `}</style>
    </div>
  );
}
